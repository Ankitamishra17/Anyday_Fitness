...
//   return (
//     <div className="relative w-full sm:w-auto sm:mx-auto">
...
//             className={`relative z-10 shrink-0 snap-start sm:snap-align-none font-rajdhani font-bold uppercase tracking-wide text-xs sm:text-sm px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-full transition-colors duration-300 whitespace-nowrap ${
...
function CategoryTabs({ categories, active, setActive }) {
  ...
  return (
    <div className="relative w-full sm:w-auto sm:mx-auto">
      ...
      <div
        ref={containerRef}
        className="relative flex flex-nowrap sm:flex-wrap items-center sm:justify-center gap-1 p-1.5 rounded-full border border-black/10 bg-surface shadow-sm backdrop-blur-sm overflow-x-auto sm:overflow-visible scroll-smooth snap-x snap-mandatory sm:snap-none [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] mx-auto max-w-full"
      >
        <span
          className="absolute top-1.5 bottom-1.5 rounded-full bg-primary transition-all duration-300 ease-out"
          style={{ left: indicator.left, width: indicator.width }}
      ...
      <section className="py-10 sm:py-16 md:py-24">
        <div className="container-x px-4 sm:px-6 lg:px-8">
          <motion.div
            ...
            className="flex flex-col items-center gap-3 sm:gap-4 mb-8 sm:mb-12"
          >
            <CategoryTabs
              categories={galleryCategories}
              active={active}
          ...
          <AnimatePresence mode="wait">
            <motion.div
              ...
              className="columns-1 xs:columns-2 sm:columns-2 lg:columns-3 xl:columns-4 gap-3 sm:gap-4 lg:gap-5 [column-fill:_balance]"
            >
              {filtered.map((image, i) => (
                <motion.div
                  key={image.id}
                  ...
                  className="mb-3 sm:mb-4 lg:mb-5 break-inside-avoid"
                >
                  <GalleryCard
                    image={image}
                    onClick={() => openLightbox(i)}
          ...
          {filtered.length === 0 && (
            <motion.div
              ...
              className="flex flex-col items-center gap-3 py-12 sm:py-16 px-4 text-center"
            >
              <p className="font-teko text-2xl sm:text-3xl text-heading uppercase">
                No Photos Found
              </p>
