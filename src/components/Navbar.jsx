...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        {/* <nav className="container-x flex items-center justify-between"> */}
        <nav className="container-x h-14 sm:h-20 flex items-center justify-between">
          <Logo />
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `relative font-rajdhani font-semibold uppercase tracking-wide text-sm pb-1 transition-colors duration-300 ${
                      isActive
                        ? "text-primary after:content-[''] after:absolute after:left-0 after:bottom-0 after:w-full after:h-[2px] after:bg-primary"
                        : "text-heading/85 hover:text-primary"
                    }`
                  }
                >
                  {link.label}
          ...
          <button
            className="lg:hidden text-heading -mr-2 p-2"
            onClick={() => setOpen(true)}
            aria-label="Open menu"
          >
            <Menu size={24} className="sm:hidden" />
      ...
      {createPortal(
        ...
        >
          ...
          />
          <div
            className={`absolute top-0 right-0 h-full w-[82%] sm:w-[60%] max-w-xs bg-surface isolate transform-gpu shadow-2xl border-l border-black/10 p-5 sm:p-6 flex flex-col gap-6 sm:gap-8 overflow-y-auto transition-transform duration-300 ${
              open ? "translate-x-0" : "translate-x-full"
            }`}
          >
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => setOpen(false)}
                aria-label="Close menu"
                className="text-heading -mr-2 p-2"
              >
                <X size={24} />
              </button>
            </div>
              ...
              {navLinks.map((link) => (
                <li key={link.to}>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `font-rajdhani font-semibold uppercase tracking-wide text-base sm:text-lg ${
                        isActive ? "text-primary" : "text-heading/85"
                      }`
                    }
                  >
                    {link.label}
