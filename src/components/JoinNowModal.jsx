...
//     try {
//       const response = await fetch("https://anydayfitness.in/send-join.php", {
//         method: "POST",
//         headers: {
...
//           Join <span className="text-primary">Anyday Fitness</span>
//         </h2>
...
//             <button
//               type="submit"
//               className="btn-primary justify-center group mt-2"
//             >
//               Submit
//               <Send
//                 size={16}
...
export default function JoinNowModal({ open, onClose }) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-md bg-surface border border-black/10 rounded-2xl shadow-2xl p-6 sm:p-8 max-h-[90vh] overflow-y-auto">
        <button
          onClick={onClose}
          aria-label="Close"
          className="absolute top-4 right-4 w-9 h-9 flex items-center justify-center rounded-full border border-black/10 text-body hover:border-primary hover:text-primary transition-colors duration-300"
        >
          <X size={18} />
        </button>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                value={form.name}
                onChange={handleChange}
                className="bg-surface2 border border-black/10 rounded-md px-4 py-3 text-sm text-offwhite font-inter focus:outline-none focus:border-primary transition-colors"
                placeholder="Your name"
              />
            </div>
